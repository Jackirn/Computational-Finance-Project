...
% === PATCH: frontiera robusta corretta (media dei pesi -> poi μ, σ) ===
mu    = ExpRet';   % NumAssets x 1 (annuale)
Sigma = V;         % NumAssets x NumAssets (annuale)

% Pesi medi sui resample (portafogli fattibili)
meanWeights = mean(WeightsSim, 3);    % [NumAssets x nPort]

% Valuta μ e σ del portafoglio medio con i parametri di riferimento
FrontierRet_rob  = meanWeights' * mu;                          % [nPort x 1]
FrontierVola_rob = sqrt(diag(meanWeights' * Sigma * meanWeights)); % [nPort x 1]

% Mantieni i nomi usati dal tuo codice a valle
meanRet  = FrontierRet_rob; 
meanRisk = FrontierVola_rob;

% (opzionale) dispersioni solo descrittive, puoi lasciarle
stdRisk = std(RiskPtfSim, 0, 2);
stdRet  = std(RetPtfSim,  0, 2);

% === Robust MVP (sulla frontiera robusta) ===
[~, idx_MVP_RF] = min(meanRisk);
ret_MVP_RF = meanRet(idx_MVP_RF);
w_MVP_RF   = meanWeights(:, idx_MVP_RF);

% === Robust MSRP (sulla frontiera robusta) ===
if ~exist('rf','var'); rf = 0; end   % assicurati che rf sia annuale
