...
w = rand(N, NumAssets); % vector of dim. = NumAssets of random numbers drawn from a uniform distr. in (0,1).
w = w./sum(w, 2); % normalize weights --> their sum has to be = 1.

RetPtfs = w*ExpRet';
VolaPtfs = sqrt(sum((w*V) .* w, 2));
SharpePtfs = RetPtfs./VolaPtfs;
